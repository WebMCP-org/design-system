...
function formatJavaScriptDisplayValue(
  ...
  const primitiveDisplayValue = formatPrimitiveDisplayValue(value);
  if (primitiveDisplayValue) return primitiveDisplayValue;
  ...
  if (Array.isArray(value)) {
    return formatDisplayArray(value, depth, seen);
  }

  return formatDisplayObject(value, depth, seen);
}

function formatPrimitiveDisplayValue(value: unknown): string | undefined {
...
function formatDisplayArray(value: unknown[], depth: number, seen: WeakSet<object>): string {
  if (value.length === 0) {
    seen.delete(value);
    return "[]";
  }
...
function formatDisplayObject(value: object, depth: number, seen: WeakSet<object>): string {
