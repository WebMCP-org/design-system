...
export function FileTree({
  ...
  return (
    <FileTreeContext.Provider value={ctx}>
      <div ref={ref} className={cx("file-tree", className)} {...props}>
        <ul className="file-tree__list" role="tree" aria-label={ariaLabel} tabIndex={0}>
          {children}
        </ul>
      </div>
...
function renderNestedFileTreeChild(child: React.ReactNode, level: number): React.ReactNode {
  if (
...
function dataFlag(enabled: boolean): "" | undefined {
  return enabled ? "" : undefined;
}

function getFileTreeNodeTabIndex(focusedPath: string | null, path: string, level: number): 0 | -1 {
  return focusedPath === path || (!focusedPath && level === 0) ? 0 : -1;
}

function getFileTreeNodeIndent(level: number, offset: number): string {
    ...
    >
      ...
      {isExpanded ? (
        <ul className="file-tree__list" role="group">
          {React.Children.map(children, (child) => renderNestedFileTreeChild(child, level))}
        </ul>
      ) : null}
    </li>
  );
